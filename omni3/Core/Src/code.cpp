...
volatile float vl53_p_gain =
    0.003f; // Gain: (e.g. 0.003 -> 100mm diff = 0.3m/s)
...
EnumMap<mechs::omni3::Id, ctls::ControlVelocity<Qty<Radian>, Qty<Ampere>> *>
    cvs;
...
void setup() {
  ...
  dji = new mods::Dji<Fdcan>(new Fdcan(&hfdcan3), DJI_SAMPLING_PERIOD,
                             DJI_DETAILS);
...
void try_vl53_init() {
  ...
  if (vl53_2->init()) {
    ...
    vl53_2->set_high_speed_mode(); // 33ms -> 20ms (50Hz)
...
void loop() {
  ...
  HAL_GPIO_WritePin(GPIOA, GPIO_PIN_4,
                    (gpio_ctl_pa4 != 0 ? GPIO_PIN_SET : GPIO_PIN_RESET));
  ...
  */
  ...
  if (joy.buttons[mods::espdbt::Button::UP]) {
    target_transform.velocity.y =
        Qty<MeterPerSecond>(-0.20_mps); // Inverted (+ -> -)
    ...
    target_transform.angvel =
        angle::shortest_angular_distance(yaw_offset, yaw) *
  ...
  // Fix Coordinate System with Mounting Offset (45 deg Left = Front)
  // 45 deg = PI/4 = 0.785398 rad
  // set_heading(-(yaw - offset - 45deg) + PI)
  ik->set_heading(-(yaw - yaw_offset - 0.7854_rad) + 3.14159265_rad);
  ik->set_transform(target_transform);
  ik->update();

  for (const mechs::omni3::Id id : AllVariants<mechs::omni3::Id>()) {
    cvs[id]->set_target_velocity(ik->get_velocity(id) / WHEEL_RADIUS);
    cvs[id]->set_now_velocity(
        dji->get_now_head_angvel(OMNI3_TO_DJI[id]).unwrap());
