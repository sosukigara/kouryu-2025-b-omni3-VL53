...
public:
    ...
    bool set_i2c_address(uint8_t new_address) {
        ...
        last_hal_status = HAL_I2C_Mem_Write(
            hi2c, address << 1, 0x0001, I2C_MEMADD_SIZE_16BIT, data_16bit.data(), 1, 2
        );
    ...
    bool write_reg16_addr(uint16_t reg, uint8_t value) {
        last_hal_status =
            HAL_I2C_Mem_Write(hi2c, address << 1, reg, I2C_MEMADD_SIZE_16BIT, &value, 1, 2);
        return last_hal_status == HAL_OK;
    }
    ...
    uint16_t read_reg16_addr(uint16_t reg) {
        uint8_t data[2] = {0, 0};
        last_hal_status =
            HAL_I2C_Mem_Read(hi2c, address << 1, reg, I2C_MEMADD_SIZE_16BIT, data, 2, 2);
        return (uint16_t)((data[0] << 8) | data[1]);
    }

    bool write_reg(uint8_t reg, uint8_t value) {
        last_hal_status = HAL_I2C_Mem_Write(hi2c, address << 1, reg, 1, &value, 1, 2);
        return last_hal_status == HAL_OK;
    }
    ...
    bool write_reg16(uint8_t reg, uint16_t value) {
        ...
        last_hal_status = HAL_I2C_Mem_Write(hi2c, address << 1, reg, 1, data, 2, 2);
        return last_hal_status == HAL_OK;
    }

    uint8_t read_reg(uint8_t reg) {
        uint8_t value = 0;
        last_hal_status = HAL_I2C_Mem_Read(hi2c, address << 1, reg, 1, &value, 1, 2);
        return value;
    }

    uint16_t read_reg16(uint8_t reg) {
        uint8_t data[2] = {0, 0};
        last_hal_status = HAL_I2C_Mem_Read(hi2c, address << 1, reg, 1, data, 2, 2);
    ...
    bool set_high_speed_mode() {
        if (is_l1x) {
            ...
            // steps). Simplified: Write 20ms macro period? For now, let's assume
            // L0X is the target for high speed (L1X is usually longer range)
            return true;
        } else {
            // L0X High Speed (~20ms)
